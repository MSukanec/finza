...
- **`categories`** (11 cols | FK: group_id → category_groups, workspace_id → workspaces, user_id → users)
...
- **`transactions`** (23 cols | FK: user_id → users, related_transaction_id → transactions, wallet_id → wallets, import_batch_id → import_batches, workspace_id → workspaces, partner_id → partners, category_id → categories)
- **`users`** (8 cols)
- **`wallet_reconciliations`** (14 cols | FK: user_id → users, workspace_id → workspaces, wallet_id → wallets, adjustment_transaction_id → transactions)
...
- `set_transaction_fingerprint()` → trigger *(public/functions_1.md)*
- `transaction_fingerprint(p_wallet uuid, p_date timestamp with time zone, p_amount numeric, p_type text, p_description text)` → text *(public/functions_1.md)*
